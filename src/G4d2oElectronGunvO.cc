...
G4d2oElectronGunvO::G4d2oElectronGunvO()
{
    G4cout << "\tConstructing G4d2oElectronGunvO..." ;
    ...
    // -------- NEW: load fluxWeight.root and fluxW histogram --------haxtonhists.root
    angleOffset = -std::atan(8.077 / 16.76);  // same offset as in readFW.C

    // Adjust path if needed
    const char* fluxPath = "./xscnData/haxtonhists.root";

    fluxFile = TFile::Open(fluxPath, "READ");
    if (!fluxFile || fluxFile->IsZombie()) {
        G4Exception("G4d2oElectronGunvO::G4d2oElectronGunvO",
                    "FluxFileError", FatalException,
                    ("Cannot open flux weight file: " + G4String(fluxPath)).c_str());
    }

fluxW = dynamic_cast<TH2D*>(fluxFile->Get("haxtondd2"));
if (!fluxW) {
    G4Exception("G4d2oElectronGunvO::G4d2oElectronGunvO",
                "FluxHistError", FatalException,
                "Histogram 'haxtondd2' not found in flux weight file.");
}

// --- NEW: clean negative bins and normalize only positive content ---
int nx = fluxW->GetNbinsX();
int ny = fluxW->GetNbinsY();

double sumPos = 0.0;
for (int ix = 1; ix <= nx; ++ix) {
    for (int iy = 1; iy <= ny; ++iy) {
        double c = fluxW->GetBinContent(ix, iy);
        if (c < 0.0) {
            // For sampling, we cannot have negative probabilities
            fluxW->SetBinContent(ix, iy, 0.0);
        } else {
            sumPos += c;
        }
    }
...
if (sumPos <= 0.0) {
    G4Exception("G4d2oElectronGunvO::G4d2oElectronGunvO",
                "FluxHistError", FatalException,
                "Histogram 'haxtondd2' has no positive content after cleaning.");
} else {
    fluxW->Scale(1.0 / sumPos);
}

G4cout << "\n[ElectronGun] Loaded haxtondd2 from " << fluxPath
       << " (pos-sum normalized to 1)" << G4endl;


    G4cout << "[EGun] haxtondd2 X: [" << fluxW->GetXaxis()->GetXmin()
       << ", " << fluxW->GetXaxis()->GetXmax()
       << "] title = " << fluxW->GetXaxis()->GetTitle() << G4endl;

G4cout << "[EGun] haxtondd2 Y: [" << fluxW->GetYaxis()->GetXmin()
       << ", " << fluxW->GetYaxis()->GetXmax()
       << "] title = " << fluxW->GetYaxis()->GetTitle() << G4endl;
...
G4d2oElectronGunvO::~G4d2oElectronGunvO()
{
	...
	G4cout<<"Deleting G4d2oElectronGunvO...";
...
void G4d2oElectronGunvO::GeneratePrimaries(G4Event* anEvent)
{
