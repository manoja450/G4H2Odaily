...
G4d2oN16Gun::G4d2oN16Gun()
{
    
    G4cout << "\tConstructing G4d2oN16Gun..." ;
...
void G4d2oN16Gun::GeneratePrimaries(G4Event* anEvent)
{
        // --- Lazy init: obtain N-16 once, after kernel init ---
    if (!n16_) {
        G4GenericIon::Definition(); // ensure GenericIon exists (safe no-op)
        auto* ionTable = G4IonTable::GetIonTable();
        n16_ = ionTable->GetIon(7, 16, 0.*keV); // N-16 ground state
        if (!n16_) {
            G4Exception("G4d2oN16Gun::GeneratePrimaries", "NoN16", FatalException,
                        "G4IonTable::GetIon(7,16,0 keV) returned nullptr.");
        }
        particleGun->SetParticleDefinition(n16_);
    }
