...
G4double G4d2oDataDrivenReflector::SampleOutgoingAngle(G4double incidentAngleDeg) const {
    G4double thetaOut = GetThetaOut(incidentAngleDeg);
    
    // Safety clamps to avoid stuck tracks
    if (std::abs(thetaOut) < fMinAngle && std::abs(thetaOut) > 0) {
        thetaOut = (thetaOut > 0) ? fMinAngle : -fMinAngle;
    }
    if (std::abs(thetaOut) > fMaxAngle) {
        thetaOut = (thetaOut > 0) ? fMaxAngle : -fMaxAngle;
    }
...
G4ThreeVector G4d2oDataDrivenReflector::GetReflectionDirection(const G4ThreeVector& incomingDirection,
    ...
    // Record for ROOT output
    RecordReflection(incidentDeg, thetaOutDeg);
...
void G4d2oDataDrivenReflector::SetRootFile(TFile* file) {
    if (fReflectionTree == nullptr && file != nullptr) {
        ...
        G4cout << "Created ReflectionTree for data-driven reflector" << G4endl;
    }
}
