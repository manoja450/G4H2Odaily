...
def plot_simulation_fit(totalPE_np, fit_result, output_dir, bin_width_pe=8, min_pe=MIN_PE_THRESHOLD):
    """Plot 1: Michel Electron Spectrum Simulation (from Jupyter)."""
    ...
    n_zero = np.sum(totalPE_np < min_pe)
    ...
    # ROOT-style histogram step plot
    step_x = []
    step_y = []
    ...
    y_smooth = gauss_with_offset(x_smooth, fit_result["A"], fit_result["mu"],
                                 fit_result["sigma"], fit_result["C"])
...
def plot_normalized_comparison(totalPE_np, real_energies, real_counts, real_centers, output_dir, bin_width_pe=8, min_pe=MIN_PE_THRESHOLD):
    """Plot 3: Normalized comparison (Simulation vs Real Data)."""
    ...
    # Normalize
    if hist_sim.sum() > 0 and hist_real.sum() > 0:
        hist_sim_norm = hist_sim / hist_sim.sum()
        hist_real_norm = hist_real / hist_real.sum()
    else:
        hist_sim_norm = hist_sim
        hist_real_norm = hist_real
    ...
    ax.set_title("Michel Electron Spectrum: Simulation vs Real Data (Normalized)", pad=12, fontweight='bold')
    ax.set_xlabel("Total P.E.", fontweight='bold')
    ax.set_ylabel(f"Normalized Counts / {bin_width_pe} P.E.", fontweight='bold')
    ax.set_axisbelow(True)
    ax.grid(True, which="major", linestyle="--", linewidth=0.6, alpha=0.4, color='gray')
    ax.set_xlim(min_pe, 1000)
    ...
    print("-"*60)
    print(f"{'Total events':<15} {hist_sim.sum():>15,} {hist_real.sum():>15,} {(hist_real.sum() - hist_sim.sum()):>+15,}")
...
def plot_high_resolution_comparison(totalPE_np, real_energies, output_dir, bin_width_pe=8, smooth_sigma=1.5, min_pe=MIN_PE_THRESHOLD):
    """Plot 4: High resolution comparison with smoothing (NO bottom ratio panel)."""
    bin_edges = np.arange(min_pe, 1000 + bin_width_pe, bin_width_pe)
    ...
    # Normalize
    y_real_norm = y_real_smoothed / y_real_raw.sum() if y_real_raw.sum() > 0 else y_real_smoothed
    y_sim_norm = y_sim_raw / y_sim_raw.sum() if y_sim_raw.sum() > 0 else y_sim_raw
    e_real_norm = np.sqrt(y_real_raw) / y_real_raw.sum() if y_real_raw.sum() > 0 else np.sqrt(y_real_raw)
    e_sim_norm = np.sqrt(y_sim_raw) / y_sim_raw.sum() if y_sim_raw.sum() > 0 else np.sqrt(y_sim_raw)
    ...
    ax.step(x_centers, y_real_norm, where='mid', color='blue', lw=2,
            label=f"Real data (smoothed, N={y_real_raw.sum():.0f})")
    ...
    ax.step(x_centers, y_sim_norm, where='mid', color='orange', lw=2,
            label=f"Simulation (N={y_sim_raw.sum():.0f})")
    ...
    y_fit_real = gauss_with_offset(x_fit_real, fit_real["A"], fit_real["mu"], fit_real["sigma"], fit_real["C"]) / y_real_raw.sum() if y_real_raw.sum() > 0 else gauss_with_offset(x_fit_real, fit_real["A"], fit_real["mu"], fit_real["sigma"], fit_real["C"])
    y_fit_sim = gauss_with_offset(x_fit_sim, fit_sim["A"], fit_sim["mu"], fit_sim["sigma"], fit_sim["C"]) / y_sim_raw.sum() if y_sim_raw.sum() > 0 else gauss_with_offset(x_fit_sim, fit_sim["A"], fit_sim["mu"], fit_sim["sigma"], fit_sim["C"])
    ...
    ax.set_ylabel(f"Norm. Counts / {bin_width_pe:.0f} P.E.")
...
def plot_single_panel_comparison(totalPE_np, real_energies, output_dir, bin_width_pe=40, min_pe=MIN_PE_THRESHOLD):
    """Plot 5: Single panel comparison (wider bins, no ratio)."""
    bin_edges = np.arange(min_pe, 1000 + bin_width_pe, bin_width_pe)
    x_centers = 0.5 * (bin_edges[:-1] + bin_edges[1:])
    ...
    y_real_norm = y_real_counts / norm_real if norm_real > 0 else y_real_counts
    y_sim_norm = y_sim_counts / norm_sim if norm_sim > 0 else y_sim_counts
    e_real_norm = np.sqrt(y_real_counts) / norm_real if norm_real > 0 else np.sqrt(y_real_counts)
    e_sim_norm = np.sqrt(y_sim_counts) / norm_sim if norm_sim > 0 else np.sqrt(y_sim_counts)
    ...
    x_fit_real = np.linspace(max(fit_real["left"], min_pe), min(1000, fit_real["right"]), 400)
    x_fit_sim = np.linspace(max(fit_sim["left"], min_pe), min(1000, fit_sim["right"]), 400)
    y_fit_real = gauss_with_offset(x_fit_real, fit_real["A"], fit_real["mu"], fit_real["sigma"], fit_real["C"]) / norm_real if norm_real > 0 else gauss_with_offset(x_fit_real, fit_real["A"], fit_real["mu"], fit_real["sigma"], fit_real["C"])
    y_fit_sim = gauss_with_offset(x_fit_sim, fit_sim["A"], fit_sim["mu"], fit_sim["sigma"], fit_sim["C"]) / norm_sim if norm_sim > 0 else gauss_with_offset(x_fit_sim, fit_sim["A"], fit_sim["mu"], fit_sim["sigma"], fit_sim["C"])
